...
输入格式：
每个测试输入包含 1 个测试用例，
第 1 行给出一个正整数 K (<100)，第 2 行给出 K 个互不相同的待验证的正整数 n (1<n≤100)的值，数字间用空格隔开。
...
def key_number(input_list: list):
    cover_set = set()
    for n in input_list:
        if n in cover_set:
            continue
            ...
            while n != 1:
                if n % 2 == 0:
                    ...
                    n = int((3 * n + 1) / 2)
                    cover_set.add(n)
    key_set = set(input_list) - cover_set
    key_list = sorted(key_set, reverse=True)
    key_str = ' '.join(str(i) for i in key_list)
    print(key_str)
...
if __name__ == '__main__':
    n = int(input())
    str_num = input()
    input_list = str_num.split(' ')
    input_list_new = [int(x) for x in input_list]
    key_number(input_list_new)
